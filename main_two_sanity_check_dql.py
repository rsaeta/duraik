import os
import sys
import torch
from pathlib import Path

from agents import RandomPlayer, DQNPlayer, UpdateArgs, DirectoryBasedExperiencedReplayWithHistory

from two_game import game, GameState, Card

dqn_agent = DQNPlayer(0)
dqn_agent.to(torch.device('cuda'))
optimizer = torch.optim.Adam(dqn_agent.parameters(), lr=1e-3)
loss_fn = torch.nn.MSELoss().to(torch.device('cuda'))
scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=100, gamma=0.99)
replay = DirectoryBasedExperiencedReplayWithHistory(Path('heads_up_histories_sanity_checks_11'))
batch_size = 1


def main(seed, save_dir=None, run_game=True):
    lowest_rank = 11
    if save_dir is None:
        save_dir = Path(f'heads_up_histories_sanity_checks_{lowest_rank}')
    os.makedirs(save_dir, exist_ok=True)
    if run_game:
        game_runner = game.GameRunner()
        game_runner.set_agents([dqn_agent, RandomPlayer(1)])
        initial_state = GameState(
            deck=[],  # [Card("S", 11), Card("H", 11), Card("D", 11), Card("C", 11)],
            visible_card=Card("S", 11),
            hands=tuple([
                tuple([Card("S", 14), Card("H", 14), Card("D", 14), Card("C", 14), Card("C", 12), Card("H", 12)]),
                tuple([Card("S", 13), Card("H", 13), Card("D", 13), Card("C", 13), Card("S", 12), Card("D", 12)]),
            ]),
            player_taking_action=0,
            defend_table=tuple(),
            attack_table=tuple(),
            defender=1,
            graveyard=tuple(),
            defender_has_taken=False,
            is_done=False,
        )
        game_runner.run(init_state=initial_state)
        run_save_dir = save_dir / f'run_{seed}'
        game_runner.save_run(run_save_dir)
    update_args = UpdateArgs(optimizer, scheduler, loss_fn, replay, 10, 0.9)
    dqn_agent.update(update_args)

    # save model, optimizer, and scheduler
    torch.save(dqn_agent.state_dict(), 'saved_models/heads_up_harder_9/dqn_agent.pt')
    torch.save(optimizer.state_dict(), 'saved_models/heads_up_harder_9/optimizer.pt')
    torch.save(scheduler.state_dict(), 'saved_models/heads_up_harder_9/scheduler.pt')


if __name__ == '__main__':
    num_iters = 1000
    x = 0
    for i in range(num_iters):
        x += main(i*3//2, run_game=False) or 0
    print(x/num_iters)
