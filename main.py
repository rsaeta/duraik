...
class DurakGame:
    ...
    def __init__(self, allow_step_back=False):
        ...
        self.player_agents = None  # list of player agents
    ...
    def step(self):
        ...
        action = self.players[player_id].choose_action(self.get_observable_state(player_id), actions)
        print(f'Step: {player_id} ({DurakAction.action_to_string(action)})')
        return self.do_step(player_id, action)
    ...
    def do_step(self, player_id: int, action_id: int):
        """
        ...
        if round_over:
            ...
            if self.is_done:
                print('game over====================')
                return self.get_observable_state(player_id)
            if len(self.deck.deck) == 0:
...
def main():
    gconfig = {'game_num_players': 3, 'lowest_card': 6}
    game = DurakGame()
